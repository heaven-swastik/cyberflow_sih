...
def build_atm_registry(atms_per_zone=6):
    ...
    """
    registry = []
    # Real RBI ATM Coordinates for Delhi (Zone A) for realistic demo
    real_delhi_atms = [
        {"bank": "SBI", "lat": 28.6315, "lng": 77.2167, "address": "Connaught Place Inner Circle, Delhi NCR"},
        {"bank": "HDFC", "lat": 28.5535, "lng": 77.2588, "address": "Nehru Place Electronics Market, Delhi NCR"},
        {"bank": "ICICI", "lat": 28.5562, "lng": 77.2066, "address": "Hauz Khas Village Area, Delhi NCR"},
        {"bank": "Axis Bank", "lat": 28.6297, "lng": 77.2205, "address": "Barakhamba Road, Delhi NCR"},
        {"bank": "PNB", "lat": 28.7041, "lng": 77.1025, "address": "Pitampura Metro Station, Delhi NCR"},
        {"bank": "Kotak Mahindra", "lat": 28.5244, "lng": 77.1855, "address": "Qutub Minar Complex, Delhi NCR"},
    ]
    ...
    for zone_id, hub in ZONE_HUBS.items():
        ...
        for i in range(atms_per_zone):
            ...
            if zone_id == "zone_a" and i < len(real_delhi_atms):
                # Use real coordinates for Delhi
                atm_data = real_delhi_atms[i]
                registry.append({
                    "atm_id": atm_id,
                    "zone_id": zone_id,
                    "bank_name": atm_data["bank"],
                ...
                # Jitter roughly within a 6-8km radius of the hub center for others.
